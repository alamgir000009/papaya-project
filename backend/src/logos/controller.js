...
exports.createOrUpdate = expressAsyncHandler(async (req, res, next) => {
  ...
  //   return res
  //     .status(400)
  //     .json({ success: false, message: "Name and logo URL are required" });
  // }

  const logoUrl = req.file.path;
  console.log("Logo URL:", logoUrl);

  const result = await service.create(name, logoUrl);
  res
    .status(201)
    .json({ success: true, message: "Logo added successfully", data: result });
});
